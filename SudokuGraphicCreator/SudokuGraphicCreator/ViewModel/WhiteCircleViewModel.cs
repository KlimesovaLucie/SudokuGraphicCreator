...
{
    /// <summary>
    /// ViewModel class for <see cref="WhiteCircle"/>.
    /// </summary>
    public class WhiteCircleViewModel : CircleViewModel
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of <see cref="WhiteCircleViewModel"/> class.
        /// </summary>
        ...
        public WhiteCircleViewModel(double width, double height, double left, double top, int rowIndex, int colIndex, SudokuElementType type,
            ElementLocationType location) :
            base(width, height, left, top)
        {
            var newElem = new WhiteCircle(left, top, rowIndex, colIndex, type, location);
        ...
        {
            if (sudokuElement == SudokuElementType.Consecutive)
            {
                AddSudokuVariant(SudokuType.Consecutive);
            }
            else if (sudokuElement == SudokuElementType.WhiteKropki)
            {
        ...
        private void RemoveVariant(ObservableCollection<SudokuElementViewModel> collection, SudokuElementType sudokuType)
        {
            ...
            {
                if (sudokuType == SudokuElementType.WhiteKropki && CountBlackKropkiElems(collection) != 0)
                {
                    return;
                }
            }
            if (sudokuType == SudokuElementType.Consecutive)
            {
                SudokuStore.Instance.Sudoku.Variants.Remove(SudokuType.Consecutive);
            }
            else if (sudokuType == SudokuElementType.WhiteKropki)
            {
        ...
        private int CountBlackKropkiElems(ObservableCollection<SudokuElementViewModel> collection)
        {
            int count = 0;
            foreach (var item in collection)
            {
                var elem = item as BlackCircleViewModel;
                if (elem != null && elem.SudokuElemType == SudokuElementType.WhiteKropki)
                {
                    count++;
                }
            }
        ...
        private void Remove(ObservableCollection<SudokuElementViewModel> collection, SudokuElementType elemType)
        {
            ...
            RemoveVariant(collection, SudokuElemType);
        }
